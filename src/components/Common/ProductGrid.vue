<template>
  <div
    class="border h-full w-full cursor-pointer hover:bg-sky-100 transition-all p-1 sm:p-4 bg-sky-100 rounded-md"
  >
    <router-link to="/overview">
      <div class="w-full p-2 text-center items-center">
        <div class="col-span-1 line-clamp-2">
          <img
            class="w-20 mx-auto"
            :src="getImagePath(product.type_id)"
            alt=""
          />
        </div>

        <Title class="col-span-3 line-clamp-2 mt-3" :titleType="'lg'">
          {{ product.title }}
        </Title>
        ...
        <div
          class="w-full mt-3 flex flex-col items-center justify-center gap-y-2 col-span-1 text-center"
        >
          <img
            class="w-5 sm:w-10"
            :src="getSituationIcon(product.situation)"
            alt=""
          />
